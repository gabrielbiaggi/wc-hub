  ...
  <div class="mx-auto max-w-[1500px] space-y-5">
    <header>
      <div class="flex gap-2">
        <StatusBadge :status="query.isError.value ? 'critical' : 'healthy'" :label="query.isError.value ? 'worker unavailable' : 'ephemeral worker'" />
    ...
    <section v-if="selected" class="grid gap-3 sm:grid-cols-3">
      <article class="rounded-xl border border-signal/20 bg-signal/[.04] p-5"><Plus class="h-4 w-4 text-signal" /><p class="mt-4 font-mono text-3xl">{{ selected.summary.add }}</p><p class="text-xs text-muted">To add</p></article>
      <article class="rounded-xl border border-warning/20 bg-warning/[.04] p-5"><RefreshCw class="h-4 w-4 text-warning" /><p class="mt-4 font-mono text-3xl">{{ selected.summary.change }}</p><p class="text-xs text-muted">To change</p></article>
      <article class="rounded-xl border border-danger/20 bg-danger/[.04] p-5"><SquareMinus class="h-4 w-4 text-danger" /><p class="mt-4 font-mono text-3xl">{{ selected.summary.destroy }}</p><p class="text-xs text-muted">To destroy</p></article>
    </section>

    <section class="grid gap-5 xl:grid-cols-[380px_1fr]">
      <aside class="overflow-hidden rounded-xl border border-line bg-panel/65">
        <header class="border-b border-line p-4"><h2 class="text-sm font-medium">Run history</h2></header>
      ...
      <article class="min-h-96 rounded-xl border border-line bg-[#070b13] p-5">
        <div class="flex items-center gap-2"><TriangleAlert v-if="selected?.summary.destroy" class="h-4 w-4 text-danger" /><FileCode2 v-else class="h-4 w-4 text-signal" /><h2 class="text-sm font-medium">Redacted plan output</h2></div>
        <pre class="mt-5 max-h-[560px] overflow-auto whitespace-pre-wrap break-words font-mono text-[11px] leading-5 text-slate-400">{{ selected?.output || 'Execute validate ou plan para visualizar a saída redigida do worker.' }}</pre>
      </article>
    </section>
  </div>
</template>

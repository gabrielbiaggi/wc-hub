...
<template>
  <div class="mx-auto max-w-[1680px] space-y-5">
    <header class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>
        <div class="flex flex-wrap gap-2"><StatusBadge :status="query.isError.value ? 'critical' : 'healthy'" :label="query.isError.value ? 'provider unavailable' : '12 repositories authorized'"/><span class="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase text-signal"><ShieldCheck class="h-3.5 w-3.5"/>admin token · explicit allowlist</span></div>
        <h1 class="mt-4 text-3xl font-semibold">GitHub delivery</h1>
    ...
    <section class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <article v-for="item in [{l:'Repositories',v:projects.length,i:Box},{l:'Active workflows',v:active,i:PlayCircle},{l:'Recent releases',v:releases.length,i:Tag},{l:'Admin access',v:projects.filter(p=>p.repository.permissions?.admin).length,i:ShieldCheck}]" :key="item.l" class="rounded-xl border border-line bg-panel/65 p-5"><component :is="item.i" class="h-4 w-4 text-muted"/><p class="mt-5 font-mono text-2xl">{{item.v}}</p><p class="text-xs text-muted">{{item.l}}</p></article>
    </section>

    <section class="grid gap-5 xl:grid-cols-[360px_1fr]">
      <aside class="overflow-hidden rounded-xl border border-line bg-panel/55">
        <header class="border-b border-line p-4"><h2 class="text-sm font-medium">Authorized repositories</h2><p class="mt-1 text-[10px] text-muted">Selecione para inspecionar todos os detalhes.</p></header>
        <div class="max-h-[620px] divide-y divide-line/60 overflow-auto">
          <button v-for="project in projects" :key="project.repository.id" :class="['w-full p-4 text-left transition-colors hover:bg-white/[.03]', selected?.repository.id === project.repository.id && 'bg-signal/[.06]']" @click="selectedRepository = project.repository.full_name">
            ...
            <div class="mt-2 flex gap-3 font-mono text-[9px] uppercase text-muted"><span>{{project.repository.language || 'mixed'}}</span><span>{{project.workflow_runs.length}} runs</span><span>{{project.releases.length}} releases</span></div>
          </button>
        </div>
      </aside>
      ...
      <div v-if="selected" class="space-y-5">
        <article class="rounded-xl border border-line bg-panel/65 p-5">
          <div class="flex flex-col justify-between gap-4 md:flex-row"><div><div class="flex items-center gap-2"><h2 class="text-lg font-medium text-white">{{selected.repository.full_name}}</h2><StatusBadge :status="selected.repository.permissions?.admin ? 'healthy' : 'warning'" :label="selected.repository.permissions?.admin ? 'admin' : 'limited'"/></div><p class="mt-2 max-w-3xl text-xs leading-5 text-muted">{{selected.repository.description || 'Sem descrição.'}}</p></div><a :href="selected.repository.html_url" target="_blank" rel="noreferrer" class="inline-flex h-9 items-center gap-2 rounded-lg border border-line px-3 text-xs text-muted hover:text-white">Abrir GitHub<ExternalLink class="h-3.5 w-3.5"/></a></div>
          <div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><div v-for="detail in [{k:'Visibility',v:selected.repository.visibility},{k:'Default branch',v:selected.repository.default_branch},{k:'Open issues',v:selected.repository.open_issues_count},{k:'Stars',v:selected.repository.stargazers_count},{k:'Size',v:`${selected.repository.size} KB`}]" :key="detail.k" class="rounded-lg border border-line/70 bg-slate-950/30 p-3"><p class="font-mono text-[9px] uppercase text-muted">{{detail.k}}</p><p class="mt-2 truncate text-sm text-slate-200">{{detail.v}}</p></div></div>
          <div class="mt-4 flex flex-wrap gap-2 font-mono text-[9px] uppercase"><span v-for="permission in permissionNames" :key="permission" :class="['rounded border px-2 py-1', selected.repository.permissions?.[permission] ? 'border-signal/20 text-signal' : 'border-line text-muted']">{{permission}}</span></div>
        </article>

        <article class="overflow-hidden rounded-xl border border-line bg-panel/65"><header class="border-b border-line p-4"><h2 class="text-sm font-medium">Workflow runs</h2></header><div class="divide-y divide-line/60"><div v-for="run in selected.workflow_runs" :key="run.id" class="grid gap-3 p-4 lg:grid-cols-[1fr_130px_190px] lg:items-center"><div class="flex gap-3"><GitBranch class="mt-0.5 h-4 w-4 text-pulse"/><div><a :href="run.html_url" target="_blank" rel="noreferrer" class="text-sm text-slate-200 hover:text-white">{{run.display_title || run.name}}</a><p class="mt-1 font-mono text-[9px] text-muted">{{run.head_branch}} · {{run.event}} · #{{run.run_number}} · {{run.head_sha.slice(0,7)}}</p></div></div><StatusBadge :status="tone(run.status,run.conclusion)" :label="run.conclusion || run.status"/><div class="flex justify-end gap-2"><Button v-if="run.status === 'completed'" size="sm" variant="outline" :disabled="action.isPending.value" @click="execute(selected.repository.full_name,run.id,'rerun')"><RotateCcw class="h-3.5 w-3.5"/>Reexecutar</Button><Button v-else size="sm" variant="danger" :disabled="action.isPending.value" @click="execute(selected.repository.full_name,run.id,'cancel')"><Square class="h-3.5 w-3.5"/>Cancelar</Button></div></div><p v-if="!selected.workflow_runs.length" class="p-10 text-center text-sm text-muted">Nenhum workflow encontrado.</p></div></article>

        <article class="overflow-hidden rounded-xl border border-line bg-panel/55"><header class="border-b border-line p-4"><h2 class="text-sm font-medium">Releases</h2></header><div class="grid gap-px bg-line/60 sm:grid-cols-2"><a v-for="release in selected.releases" :key="release.id" :href="release.html_url" target="_blank" rel="noreferrer" class="bg-panel p-4 hover:bg-slate-900"><div class="flex items-center gap-2"><Tag class="h-3.5 w-3.5 text-signal"/><p class="text-sm text-slate-200">{{release.name || release.tag_name}}</p></div><p class="mt-2 font-mono text-[9px] text-muted">{{release.tag_name}} · {{release.prerelease ? 'prerelease' : 'stable'}}</p></a><p v-if="!selected.releases.length" class="col-span-2 p-8 text-center text-xs text-muted">Nenhuma release publicada.</p></div></article>
      </div>
    </section>
  </div>
</template>

...
<template>
  <div class="mx-auto max-w-[1500px] space-y-5">
    <header class="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><div class="flex items-center gap-2 font-mono text-[10px] uppercase text-signal"><ShieldCheck class="h-3.5 w-3.5"/>constrained writable root</div><h1 class="mt-3 text-3xl font-semibold">MergerFS browser</h1><p class="mt-2 text-sm text-muted">Navegação, upload, organização, indexação e streaming dentro da raiz autorizada.</p></div><div class="flex flex-wrap gap-2"><input ref="input" type="file" class="hidden" @change="selectedFile"/><Button variant="outline" :disabled="mutation.isPending.value" @click="input?.click()"><Upload class="h-4 w-4"/>Upload</Button><Button variant="outline" :disabled="mutation.isPending.value" @click="mkdir"><FolderPlus class="h-4 w-4"/>Nova pasta</Button><Button variant="outline" :disabled="indexing" @click="runIndex"><Search class="h-4 w-4"/>{{indexing?'Indexando…':'Indexar'}}</Button><Button variant="outline" @click="query.refetch()"><RefreshCw class="h-4 w-4"/>Atualizar</Button></div></header>
    ...
    <div v-if="query.isError.value||mutation.isError.value" class="rounded-xl border border-danger/20 bg-danger/5 p-4 text-sm text-danger">A operação de storage falhou. Pastas só podem ser excluídas quando vazias e todos os caminhos devem permanecer dentro da raiz.</div>
    <section class="overflow-hidden rounded-xl border border-line bg-panel/65">
      <header class="flex items-center gap-2 border-b border-line p-4 font-mono text-[10px]"><button class="cursor-pointer text-signal" @click="path=''">ROOT</button><template v-for="(crumb,index) in crumbs" :key="`${crumb}-${index}`"><ChevronRight class="h-3 w-3 text-muted"/><button class="cursor-pointer text-slate-300 hover:text-white" @click="go(index)">{{crumb}}</button></template></header>

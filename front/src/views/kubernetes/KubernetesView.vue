...
import { Activity, Boxes, RefreshCw, Server, ShieldCheck, TriangleAlert } from 'lucide-vue-next'
import Button from '@/components/ui/Button.vue'
import StatusBadge from '@/components/ui/StatusBadge.vue'
import { getKubernetesOverview, type KubeNode } from '@/lib/api_kubernetes'
...
<template>
  <div class="mx-auto max-w-[1680px] space-y-5">
    <header class="flex flex-col justify-between gap-4 md:flex-row md:items-end">
      <div>
        <div class="flex gap-2"><StatusBadge :status="query.isError.value ? 'critical' : 'healthy'" :label="query.isError.value ? 'cluster unavailable' : 'service account read-only'" /><span class="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase text-signal"><ShieldCheck class="h-3.5 w-3.5" />Kubernetes RBAC</span></div>
    ...
    <section class="grid gap-5 xl:grid-cols-[1fr_1.2fr]">
      ...
      <article class="overflow-hidden rounded-xl border border-line bg-panel/65">
        ...
        <div class="divide-y divide-line/60"><div v-for="deployment in data?.deployments" :key="deployment.metadata.uid" class="grid grid-cols-[1fr_100px_110px] items-center p-4"><div><p class="text-sm text-slate-200">{{ deployment.metadata.name }}</p><p class="mt-1 font-mono text-[9px] text-muted">{{ deployment.metadata.namespace }}</p></div><p class="font-mono text-xs text-muted">{{ deployment.status.readyReplicas ?? 0 }} / {{ deployment.spec.replicas ?? 0 }}</p><StatusBadge :status="(deployment.status.readyReplicas ?? 0) >= (deployment.spec.replicas ?? 0) ? 'healthy' : 'warning'" label="replicas" /></div></div>
      </article>
    </section>

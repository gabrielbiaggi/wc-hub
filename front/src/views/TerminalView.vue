...
<template>
  <div class="mx-auto max-w-[1680px] space-y-5">
    <header class="flex items-end justify-between"><div><div class="flex items-center gap-2 font-mono text-[10px] uppercase tracking-widest text-signal"><ShieldCheck class="h-3.5 w-3.5"/>one-use ticket · known_hosts enforced</div><h1 class="mt-3 text-3xl font-semibold tracking-tight">Remote terminal</h1><p class="mt-2 text-sm text-muted">SSH intermediado pelo backend, sem chave privada no navegador.</p></div><StatusBadge :status="connected?'healthy':'warning'" :label="connected?'connected':'disconnected'"/></header>
    <section class="h-[calc(100vh-210px)] min-h-[560px] overflow-hidden rounded-xl border border-line bg-[#070b13]"><Splitpanes class="default-theme"><Pane :size="24" :min-size="18" :max-size="35"><aside class="h-full border-r border-line bg-panel/60 p-4"><div class="flex items-center gap-2"><Monitor class="h-4 w-4 text-pulse"/><h2 class="text-sm font-medium">Connection</h2></div><form class="mt-5 space-y-4" @submit.prevent="connect"><label class="block text-xs text-muted">Remote target<select v-model="hostID" required class="field"><option value="" disabled>Selecione um host</option><option v-for="host in targets" :key="host.id" :value="host.id">{{host.name}}</option></select></label><label class="block text-xs text-muted">Digite o nome exato<input v-model="confirmation" required class="field" :placeholder="selected?.name||'target-name'" autocomplete="off"/></label><label class="block text-xs text-muted">Código TOTP<input v-model="code" required class="field font-mono tracking-[.25em]" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" placeholder="000000" autocomplete="one-time-code"/></label><p v-if="error" class="rounded-lg border border-danger/20 bg-danger/5 p-3 text-[11px] leading-5 text-danger">{{error}}</p><Button v-if="!connected" class="w-full" type="submit"><PlugZap class="h-4 w-4"/>Emitir ticket e conectar</Button><Button v-else class="w-full" variant="danger" type="button" @click="disconnect">Encerrar sessão</Button></form><div class="mt-6 border-t border-line pt-4"><div class="flex items-start gap-2 text-[11px] leading-5 text-muted"><LockKeyhole class="mt-0.5 h-3.5 w-3.5 shrink-0 text-signal"/>Self target e hosts locais não aparecem nesta lista.</div></div></aside></Pane><Pane :size="76"><div class="relative flex h-full flex-col"><div class="flex h-10 items-center border-b border-line px-4 font-mono text-[10px] text-muted"><TerminalSquare class="mr-2 h-3.5 w-3.5"/>{{selected?.name||'no-session'}}<span class="ml-auto">xterm-256color</span></div><div ref="terminalRoot" class="min-h-0 flex-1 p-2"/><div v-if="!connected" class="pointer-events-none absolute inset-0 grid place-items-center"><div class="text-center"><TerminalSquare class="mx-auto h-8 w-8 text-slate-700"/><p class="mt-3 font-mono text-xs text-muted">Awaiting secure ticket</p></div></div></div></Pane></Splitpanes></section>
  </div>
</template>

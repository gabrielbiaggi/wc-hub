...
export const rollbackProxmoxSnapshot=async(cluster:string,node:string,kind:'qemu'|'lxc',vmid:number,name:string,headers?:Record<string,string>)=>(await api.post(`/v1/proxmox/nodes/${encodeURIComponent(node)}/${kind}/${vmid}/snapshots/${encodeURIComponent(name)}/rollback`,{},{params:{cluster},headers})).data
export const deleteProxmoxSnapshot=async(cluster:string,node:string,kind:'qemu'|'lxc',vmid:number,name:string,headers?:Record<string,string>)=>api.delete(`/v1/proxmox/nodes/${encodeURIComponent(node)}/${kind}/${vmid}/snapshots/${encodeURIComponent(name)}`,{params:{cluster},headers})
...
export interface DockerHealth{reachable:boolean;source?:string;version?:string;api_version?:string;os_type?:string;arch?:string}
...
export interface DockerInventory{captured_at:string;source?:string;health:DockerHealth;containers:DockerContainer[];images:DockerImage[];stats:DockerContainerStats[];warnings:string[]}

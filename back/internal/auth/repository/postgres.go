...
func (r *Postgres) EnsureDevelopmentMaster(ctx context.Context, email string) (domain.User, error) {
	...
	// Upgrade the old development identity in place so TOTP enrollment and audit
	// ownership survive the move to the real operator email.
	if _, err = tx.Exec(ctx, `UPDATE users SET email=$1,updated_at=now() WHERE email=$2 AND NOT EXISTS (SELECT 1 FROM users WHERE email=$1)`, email, domain.DevelopmentMasterIdentity); err != nil {
		return domain.User{}, err
	}
	err = tx.QueryRow(ctx, `
		INSERT INTO users (email,display_name,password_hash)
		VALUES ($1,'All Might','!dynamic-master-has-no-password-hash!')
		ON CONFLICT (email) DO UPDATE SET display_name=EXCLUDED.display_name,disabled_at=NULL,updated_at=now()
		RETURNING id::text,email,display_name,totp_enabled`, email).
	...
	var otherUsers int
	if err = tx.QueryRow(ctx, `SELECT count(*) FROM users WHERE id<>$1`, user.ID).Scan(&otherUsers); err != nil {
		return domain.User{}, err
	}
	if otherUsers != 0 {
		return domain.User{}, fmt.Errorf("single-user master mode requires removing %d additional user(s)", otherUsers)
	}

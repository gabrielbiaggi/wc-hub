...
func MountRoutes(mux *http.ServeMux, authMiddleware AuthMiddleware, handler *Handler) {
	...
	mux.HandleFunc("GET /api/v1/cloudflare/accounts/{account_id}/private-routes", authMiddleware(permission, handler.PrivateRoutes))
	mux.HandleFunc("POST /api/v1/cloudflare/accounts/{account_id}/private-routes", authMiddleware("cloudflare.manage", handler.CreatePrivateRoute))
	...
	mux.HandleFunc("GET /api/v1/cloudflare/accounts/{account_id}/workers", authMiddleware(permission, handler.ListWorkers))
	mux.HandleFunc("PUT /api/v1/cloudflare/accounts/{account_id}/workers/{name}", authMiddleware("cloudflare.manage", handler.UploadWorker))
	mux.HandleFunc("DELETE /api/v1/cloudflare/accounts/{account_id}/workers/{name}", authMiddleware("cloudflare.manage", handler.DeleteWorker))
	mux.HandleFunc("GET /api/v1/cloudflare/accounts/{account_id}/kv", authMiddleware(permission, handler.ListKVNamespaces))
	mux.HandleFunc("POST /api/v1/cloudflare/accounts/{account_id}/kv", authMiddleware("cloudflare.manage", handler.CreateKVNamespace))
	mux.HandleFunc("DELETE /api/v1/cloudflare/accounts/{account_id}/kv/{id}", authMiddleware("cloudflare.manage", handler.DeleteKVNamespace))
	mux.HandleFunc("GET /api/v1/cloudflare/zones/{zone_id}/waf", authMiddleware(permission, handler.ListWAFRules))
	mux.HandleFunc("POST /api/v1/cloudflare/zones/{zone_id}/waf", authMiddleware("cloudflare.manage", handler.CreateWAFRule))
	mux.HandleFunc("DELETE /api/v1/cloudflare/zones/{zone_id}/waf/{id}", authMiddleware("cloudflare.manage", handler.DeleteWAFRule))
	mux.HandleFunc("GET /api/v1/cloudflare/zones/{zone_id}/analytics", authMiddleware(permission, handler.ZoneAnalytics))
}

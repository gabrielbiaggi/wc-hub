...
| Proxmox `ai` | `https://100.96.36.16:8006` (Tailscale) | stable management/API endpoint; LAN fallback `192.168.68.124:8006` |
...
The pfSense VM routes its `10.20.40.0/24` LAN, but its WAN upstream and the operator LAN use different physical gateways. The Hub uses the stable Tailscale Prox-AI management endpoint; external browser access terminates through the existing `prox-ai` Cloudflare Tunnel. Do not reintroduce the retired `10.10.50.21` address.
